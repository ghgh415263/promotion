...
# 📋 Loki 기반 로그 어그리게이션 (Log Aggregation)

## 왜 필요한가 — 로그 통합 관리

마이크로서비스 환경에서는 요청 하나가 여러 서비스를 거치며 처리된다.
...
문제가 발생했을 때 각 서버에 접속하여 로그를 확인하는 것은 매우 비효율적이다.
...
서비스 수가 증가할수록 로그 추적과 장애 분석이 어려워진다.

> 💡 **로그 어그리게이션의 해결책**
>
> "모든 서비스 로그를 중앙 저장소에 수집하여 한 곳에서 검색하고 분석한다."
...
각 서비스는 JSON 형식으로 로그를 출력한다.

Grafana Alloy가 Docker 컨테이너 로그를 수집하여 Loki로 전송하고, Grafana에서 이를 조회한다.
...
애플리케이션 로그를 JSON 형태로 출력한다.

예시

```json
{
  "timestamp":"2026-06-12T10:30:15.123Z",
  "level":"INFO",
  "service":"user-service",
  "traceId":"d4f9d7f0b9f1d4d2",
  "message":"User login success"
}
```

TraceId를 함께 기록하여 OpenTelemetry Trace와 로그를 연계할 수 있다.
...
OpenTelemetry는 요청의 호출 흐름을 추적하고,

Loki는 해당 요청에서 발생한 상세 로그를 제공한다.
...
이를 통해 요청 흐름과 로그를 함께 분석하여 장애 원인을 빠르게 파악할 수 있다.
...
| 로그 수집      | OpenTelemetry Java Agent + Loki   |
| 빌드         | Gradle                            |
